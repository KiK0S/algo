...
static void test_fibonacci_recurrence() {
  ...
  const std::vector<Mint> coefficients = berlekamp_massey(sequence);
  ...
  for (int i = 0; i < static_cast<int>(sequence.size()); ++i) {
    assert(linear_recurrence_kth(initial, coefficients, i) == sequence[i]);
    assert(berlekamp_massey_kth(sequence, i) == sequence[i]);
  }

  assert(linear_recurrence_kth(initial, coefficients, 30).value() == 832040);
  assert(berlekamp_massey_kth(sequence, 30).value() == 832040);
}
...
static void test_geometric_progression() {
  ...
  Mint current(7);
  for (int i = 0; i < 12; ++i) {
    assert(linear_recurrence_kth(initial, coefficients, i) == current);
    current *= Mint(3);
  }
}
...
static void test_zero_sequence() {
  ...
  assert(linear_recurrence_kth(sequence, coefficients, 100) == Mint(0));
  assert(berlekamp_massey_kth(sequence, 100) == Mint(0));

  const std::vector<Mint> empty;
  assert(berlekamp_massey(empty).empty());
  assert(linear_recurrence_kth(empty, empty, 5) == Mint(0));
  assert(berlekamp_massey_kth(empty, 5) == Mint(0));
}
...
static void test_random_recurrences() {
  ...
  for (int it = 0; it < 250; ++it) {
    ...
    const std::vector<Mint> recovered = berlekamp_massey(prefix);
    ...
    for (int i = 0; i < total_length; ++i) {
      assert(linear_recurrence_kth(recovered_initial, recovered, i) == full[i]);
      assert(berlekamp_massey_kth(prefix, i) == full[i]);
    }
  }
}
