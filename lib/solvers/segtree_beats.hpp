...
class SegmentTreeBeats {
 public:
  ...
  T query_sum(int left, int right) {
    ...
    return sum_rec(1, 0, n_ - 1, left, right);
  }
  ...
  T query_min(int left, int right) {
    ...
    return min_rec(1, 0, n_ - 1, left, right);
  }
  ...
  T query_max(int left, int right) {
    ...
    return max_rec(1, 0, n_ - 1, left, right);
  }
 ...
 private:
  ...
  T sum_rec(int v, int tl, int tr, int l, int r) {
    if (tl > r || tr < l) {
      return T(0);
    }
    if (l <= tl && tr <= r) {
    ...
    push(v, tl, tr);
    const int tm = (tl + tr) / 2;
    return sum_rec(v * 2, tl, tm, l, r) +
           sum_rec(v * 2 + 1, tm + 1, tr, l, r);
  }

  T min_rec(int v, int tl, int tr, int l, int r) {
    if (tl > r || tr < l) {
      return pos_inf();
    }
    if (l <= tl && tr <= r) {
    ...
    push(v, tl, tr);
    const int tm = (tl + tr) / 2;
    return std::min(min_rec(v * 2, tl, tm, l, r),
                    min_rec(v * 2 + 1, tm + 1, tr, l, r));
  }

  T max_rec(int v, int tl, int tr, int l, int r) {
    if (tl > r || tr < l) {
      return neg_inf();
    }
    if (l <= tl && tr <= r) {
    ...
    push(v, tl, tr);
    const int tm = (tl + tr) / 2;
    return std::max(max_rec(v * 2, tl, tm, l, r),
                    max_rec(v * 2 + 1, tm + 1, tr, l, r));
  }
};
