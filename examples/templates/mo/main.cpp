...
struct MoQuery {
  ...
  friend std::istream& operator>>(std::istream& in, MoQuery& query) {
    return in >> query.left >> query.right;
  }

  friend std::ostream& operator<<(std::ostream& out, const MoQuery& query) {
    return out << query.left << ' ' << query.right;
  }
};
