...
// Catalog entry: /templates/halfplane_intersection
...
enum class LineIntersectionType { none, point, coincident };

template <typename T>
inline LineIntersectionType line_intersection(
  ...
  if (std::fabs(denom) <= eps) {
    return std::fabs(cross_ld(cc - aa, ab)) <= eps
...
inline std::vector<Point2<long double>> halfplane_intersection(
  ...
  EDULCNI_VIS(edulcni::live::halfplanes(
      "halfplane.constraints", halfplanes));
  EDULCNI_VIS(edulcni::live::scalar(
      "halfplane.count", static_cast<std::uint64_t>(halfplanes.size())));
  EDULCNI_STEP("Half-plane intersection initialized");
  ...
  for (const HalfPlane& plane : halfplanes) {
    ...
    EDULCNI_VIS(edulcni::live::halfplanes(
        "halfplane.constraints", deque_planes));
    EDULCNI_VIS(edulcni::live::points_overlay(
        "halfplane.constraints", deque_intersections));
    EDULCNI_STEP("Half-plane intersection accepted a boundary");
  }
  ...
  std::vector<Point2<long double>> result(deque_intersections.begin(),
                                          deque_intersections.end());
  EDULCNI_VIS(edulcni::live::halfplanes(
      "halfplane.constraints", deque_planes));
  EDULCNI_VIS(edulcni::live::polygon_overlay(
      "halfplane.constraints", result));
  EDULCNI_STEP("Half-plane intersection completed");
  return result;
}
