...
PATTERNS = [
    ...
    Pattern("read_array_indexed", r"(forn\s*\([^\n]*\)|for\s*\([^\n]*i\s*=\s*0[^\n]*i\s*<\s*n[^\n]*\))[^\n]*cin\s*>>\s*\w+\s*\[\s*i\s*\]", "/templates/read_array", "template", 2),
    Pattern("read_vector_by_ref", r"for\s*\(\s*auto\s*&\s*\w+\s*:\s*\w+\s*\)\s*cin\s*>>", "/templates/read_vector_ref", "template", 1),
    Pattern("decrement_indices", r"cin\s*>>[^;]+;\s*(--\w+|\w+--)|cin\s*>>[^;]+,\s*\w+--", "/templates/decrement_indices", "template", 1),
    Pattern("undirected_edges", r"g\s*\[\s*\w+\s*\]\s*\.push_back\s*\(\s*\w+\s*\)\s*;\s*g\s*\[\s*\w+\s*\]\s*\.push_back\s*\(\s*\w+\s*\)", "/templates/read_graph_undirected", "template", 2),
